// tb_blitter_trilist_calpha.sv — BLT_OP_TRILIST end-to-end equivalence test.
//
// Clones the DDR + P_SRC source models of tb_blitter_copy_pipe.sv, but drives a
// full TRILIST scene from vectors/tri_calpha_ddr.hex (generated by gen_tri_golden.c)
// and diffs the ENTIRE composited comp_fbram against vectors/tri_calpha_exp.hex
// (`FB_PIXELS RGB565 pixels) within +-1 LSB per channel.
//
// The control block (@0x200000) requests a CLEAR-before-list (blue bg) followed
// by one textured triangle with blend_mode=CONST_ALPHA and PER-VERTEX alpha
// (interpolated ca) combined with the global c_alpha, exercising blt_blend's
// (ca*g_alpha)/255 combine AND the comp_fbram dst-read latency path
// (S_TRI_DSTW/DSTC); the vertex array + texture live at SRC (@0x210000). The
// tri walk composites into comp_fbram via the tri_busy bus muxes.
`timescale 1ns/1ps
`default_nettype none
`include "blitter_defs.vh"
module tb_blitter_trilist_calpha;
  ...
  initial begin
    ...
    $readmemh("vectors/tri_calpha_ddr.hex", mem);
    $readmemh("vectors/tri_calpha_exp.hex", exp);
  end
