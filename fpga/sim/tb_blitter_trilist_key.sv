// tb_blitter_trilist_key.sv — BLT_OP_TRILIST end-to-end equivalence test.
//
// Clones the DDR + P_SRC source models of tb_blitter_copy_pipe.sv, but drives a
// full TRILIST scene from vectors/tri_key_ddr.hex (generated by gen_tri_golden.c)
// and diffs the ENTIRE composited comp_fbram against vectors/tri_key_exp.hex
// (`FB_PIXELS RGB565 pixels) within +-1 LSB per channel.
//
// The control block (@0x200000) requests a CLEAR-before-list (blue bg) followed
// by one textured triangle whose 8x8 texture is split black|green (left|right
// half) with blend_mode=COLORKEY keyed on black, exercising write_en=0 cull on
// the keyed texels; the vertex array + texture live at SRC (@0x210000). The tri
// walk composites into comp_fbram via the tri_busy bus muxes.
`timescale 1ns/1ps
`default_nettype none
`include "blitter_defs.vh"
module tb_blitter_trilist_key;
  ...
  initial begin
    ...
    $readmemh("vectors/tri_key_ddr.hex", mem);
    $readmemh("vectors/tri_key_exp.hex", exp);
  end
