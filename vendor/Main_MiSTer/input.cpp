...
/* Maldita joy-shm passthrough: when enabled, input_poll() mirrors each
 * player's finalized joy_mask (| autofire) into joy_mask_export[], which
 * input_get_joy_mask() exposes to the wrapper's /dev/shm publisher. */
...
/* Maldita joy-shm publisher API (extern "C" — maldita_joy_shm.cpp forward-
 * declares both this way, so the linkage must match exactly). */
extern "C" void input_get_joy_mask(uint32_t *masks, int n)
{
	for (int i = 0; i < n && i < NUMPLAYERS; i++)
		masks[i] = joy_mask_export[i];
}

extern "C" void input_set_joy_passthrough(int on)
{
	input_joy_passthrough = on;
}
