...
// MALDITA OVERLAY — the entire local change to this file is this variable, the
// four lines inside user_io_status_set() below, and user_io_status_trigger_take()
// after it. Everything else is upstream verbatim (Main_MiSTer @3380931);
// build-hps.sh's verify_overlay_is_additive() enforces that.
//
// WHY IT IS NEEDED. A CONF_STR "T" option (ours is "TJ,Reset;" = status bit 19)
// is a MOMENTARY trigger: menu.cpp:2503-2504 calls user_io_status_set(opt, 1)
// immediately followed by user_io_status_set(opt, 0), both inside one HandleUI()
// call. Nothing outside that call can ever observe the bit set by reading
// status — by the time the scheduler switches back to co_poll it is already 0.
// So the pulse has to be captured where it is generated. This latches it sticky;
// maldita_hook.cpp consumes and clears it with the take() below.
//
// ex==0 only: lowercase 't'/'r' options address the EXTENDED status word, whose
// bit numbering is a different 32-bit space. Capturing those here would alias a
// "t19" onto our "T19". Maldita's CONF_STR has no lowercase trigger, so this
// guard costs nothing today and stops a future one from firing a spurious Reset.
...
void user_io_status_set(const char *opt, uint32_t value, int ex)
{
	...
	// MALDITA OVERLAY: latch a single-bit set-to-1 (a T/R pulse) as sticky.
	if (!ex && size == 1 && value == 1) g_status_trigger_flags |= (1u << start);
...
// MALDITA OVERLAY. Consume-and-clear the trigger pulses latched since the last
// call. Read-then-zero without a lock is safe here because both the writer
// (HandleUI, on co_ui) and the reader (maldita_hook_poll, on co_poll) are
// cothreads on ONE OS thread that only switch at explicit scheduler_yield()
// points — there is no preemption between the read and the store. A pulse
// generated between them would be lost, but there is no such window.
