...
- **Run the game:** `cd /media/fat/games/gmloader && /media/fat/Scripts/gmloader_diag.sh --preset fabric`. The fabric path REQUIRES both `GMLOADER_BLITTER=2` and `GMLOADER_RASTER=mfgpu` (the `--preset fabric` sets them + profiling env). Output log → `/tmp/gmloader.log`.
