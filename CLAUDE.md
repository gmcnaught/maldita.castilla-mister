...
Parallel track on branch `claude/engine-daemon-refactor-fqodfd`: **HPS takeover** — stop being a guest process on a machine `Main_MiSTer` owns (DreamSTer's model, taken *late*: MiSTer loads the core and satisfies its readiness contract, then we kill it once the engine is proven live on the fabric). Spec `docs/superpowers/specs/2026-08-04-hps-takeover-launcher-design.md`, plan `docs/superpowers/plans/2026-08-04-hps-takeover-launcher.md`. A **daemon-free entry point** landed with it — `Scripts/MalditaCastilla.sh` writes `load_core` to `/dev/MiSTer_cmd`, waits for `/tmp/CORENAME`, then execs the ordinary `_handler.sh`, so `Master_Daemon` is out of the launch path entirely (MiSTer services that FIFO while a Scripts entry runs, and MiSTer — not the daemon — writes `/tmp/CORENAME`). The harness (`games/Maldita Castilla/mister_takeover.sh`, `deploy.py --takeover`) is landed but **default-off and not yet device-run**; it is blocked for real use on an engine-side forced input transport, because input reaches the game through `Main_MiSTer` → `hps_io` → the fabric, and dies frozen when MiSTer does. What it unlocks: both cores, cpufreq, and a write-combining DDR mapping (`mem_wc`) for the texture-staging path.
