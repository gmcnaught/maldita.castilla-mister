#!/usr/bin/env bash
# Build tools/mister-mem-wc/prebuilt/mem_wc-<vermagic>.ko for a MiSTer kernel.
#
# Ported from solarus-mister/scripts/build_mem_wc.sh, which built the first
# 6.18.38-MiSTer object; only the paths differ.
#
#   tools/mister-mem-wc/build_prebuilt.sh --host 192.168.20.81        # config from the device
#   tools/mister-mem-wc/build_prebuilt.sh --config path/to/config --release 6.18.38-MiSTer
...
cp /src/tools/mister-mem-wc/mem_wc.c /tmp/m/
...
out=/src/tools/mister-mem-wc/prebuilt/mem_wc-$rel.ko
